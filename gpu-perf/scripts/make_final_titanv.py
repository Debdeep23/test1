...
def main():
    ...
    # 2) read GPU files
    props_txt  = open(os.path.join(DATA,"props_titanv.out")).read()
    stream_txt = open(os.path.join(DATA,"stream_like_titanv.out")).read()
    gemm_txt   = open(os.path.join(DATA,"gemm_cublas_titanv.out")).read()
    ...
    # 3) write final CSV with updated schema (matches current runs_titanv_final.csv)
    fieldnames = [
    ...
    out_path = os.path.join(DATA,"runs_titanv_final.csv")
